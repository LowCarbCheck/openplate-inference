...
The two sibling services grew one in August 2026 — `openplate-gateway` has
`gw-api` over its member and invite endpoints, and `openplate-sync` has
`sync-api` over an account-metadata surface. This service deliberately grew
neither, and the reason is worth writing down so the absence reads as a decision
rather than an oversight.

**This service implements someone else's specification.** Its surface is the
OpenAI chat-completions shape, which is what lets any OpenAI-compatible client —
openplate included — point at it with no adapter. An API is "first" here in the
...
state that dies with the process. `/readyz` already answers the only operational
question anyone asks — can it serve a scan right now — and it answers it without
a credential, which is what a monitoring probe needs.
...
If this service ever grows durable per-caller state — per-key quotas, a usage
ledger, anything that must be listed or revoked — this decision should be
revisited, and that is the trigger to watch for.
